  ...
  <div class="mx-2">
    ...
    <table class="table table-striped borda" cellpadding="6">
      ...
      <tbody>
        ...
        //totalizar os vendas 
        $query = $pdo->query("SELECT * FROM tb_vendas where data >= '$dataInicial' and data <= '$dataFinal' and pago = 'Sim' and status != 'Cancelado'");
        ...
        $query = $pdo->query("SELECT * FROM tb_receber where data_pgto >= '$dataInicial' and data_pgto <= '$dataFinal' and tipo = 'Conta' and pago = 'Sim' ORDER BY data_pgto asc");
        $res = $query->fetchAll(PDO::FETCH_ASSOC);
        $total_reg = @count($res);
        for ($i = 0; $i < $total_reg; $i++) {
          foreach ($res[$i] as $key => $value) {
        ...
        $query = $pdo->query("SELECT * FROM tb_pagar where data_pgto >= '$dataInicial' and data_pgto <= '$dataFinal' and tipo = 'Conta' and pago = 'Sim' ORDER BY data_pgto asc");
        $res = $query->fetchAll(PDO::FETCH_ASSOC);
        $total_reg = @count($res);
        for ($i = 0; $i < $total_reg; $i++) {
          foreach ($res[$i] as $key => $value) {
        ...
        $query = $pdo->query("SELECT * FROM tb_pagar where data_pgto >= '$dataInicial' and data_pgto <= '$dataFinal' and tipo = 'Compra' and pago = 'Sim' ORDER BY data_pgto asc");
        $res = $query->fetchAll(PDO::FETCH_ASSOC);
        $total_reg = @count($res);
        for ($i = 0; $i < $total_reg; $i++) {
          foreach ($res[$i] as $key => $value) {
