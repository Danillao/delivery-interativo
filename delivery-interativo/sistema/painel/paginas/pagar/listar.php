...
HTML;
  ...
  for ($i = 0; $i < $total_reg; $i++) {
    ...
    $query2 = $pdo->query("SELECT * FROM tb_fornecedores where id = '$pessoa'");
    ...
    $query2 = $pdo->query("SELECT * FROM tb_usuarios where id = '$funcionario'");
    $res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
    $total_reg2 = @count($res2);
    if ($total_reg2 > 0) {
...
<td>
      ...
      <p>Confirmar Baixa na Conta? <a href="#" onclick="baixar('{$id}')"><span class="text-verde {$visivel}">Sim</span></a></p>
		...
		<big><a href="http://api.whatsapp.com/send?1=pt_BR&phone=$whats&text=" target="_blank" title="Abrir Whatsapp" class="{$classe_whats}"><i class="fab fa-whatsapp text-verde"></i></a></big>
