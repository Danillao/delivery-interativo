...
$query = $pdo->query("SELECT * FROM tb_clientes ");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$total_clientes = @count($res);


$query = $pdo->query("SELECT * FROM tb_pagar where data_venc = '$data_atual' and pago != 'Sim' ");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$contas_pagar_hoje = @count($res);


$query = $pdo->query("SELECT * FROM tb_receber where data_venc = '$data_atual' and pago != 'Sim' ");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$contas_receber_hoje = @count($res);


$query = $pdo->query("SELECT * FROM tb_produtos where tem_estoque = 'Sim'");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$total_reg = @count($res);
$estoque_baixo = 0;
if ($total_reg > 0) {
...
$total_debitos_dia = 0;
$query = $pdo->query("SELECT * FROM tb_pagar where data_pgto = '$data_atual'");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
if (@count($res) > 0) {
...
$total_ganhos_dia = 0;
$query = $pdo->query("SELECT * FROM tb_receber where data_pgto = '$data_atual' ");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
if (@count($res) > 0) {
  for ($i = 0; $i < @count($res); $i++) {
    foreach ($res[$i] as $key => $value) {
...
//totalizar os vendas 
$total_vendas_dia = 0;
$query = $pdo->query("SELECT * FROM tb_vendas where data = '$data_atual' and pago = 'Sim' and status != 'Cancelado'");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$total_reg = @count($res);
for ($i = 0; $i < $total_reg; $i++) {
  foreach ($res[$i] as $key => $value) {
...
for ($i = 1; $i <= 12; $i++) {
  ...
  //DESPESAS
  $total_mes_despesa = 0;
  $query = $pdo->query("SELECT * FROM tb_pagar where pago = 'Sim' and data_pgto >= '$data_mes_inicio_grafico' and data_pgto <= '$data_mes_final_grafico' ORDER BY id asc");
  $res = $query->fetchAll(PDO::FETCH_ASSOC);
  $total_reg = @count($res);
  if ($total_reg > 0) {
  ...
  //VENDAS
  $total_mes_vendas = 0;
  $query = $pdo->query("SELECT * FROM tb_vendas where pago = 'Sim' and status != 'Cancelado' and data >= '$data_mes_inicio_grafico' and data <= '$data_mes_final_grafico'");
  $res = $query->fetchAll(PDO::FETCH_ASSOC);
  $total_reg = @count($res);
  if ($total_reg > 0) {
    for ($i2 = 0; $i2 < $total_reg; $i2++) {
