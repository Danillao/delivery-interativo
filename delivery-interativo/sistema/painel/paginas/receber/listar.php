...
HTML;
  ...
  for ($i = 0; $i < $total_reg; $i++) {
    ...
    $query2 = $pdo->query("SELECT * FROM tb_clientes where id = '$pessoa'");
    ...
    $query2 = $pdo->query("SELECT * FROM tb_usuarios where id = '$usuario_lanc'");
    $res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
    $total_reg2 = @count($res2);
    if ($total_reg2 > 0) {
...
<td>
      ...
      <p>Confirmar Baixa na Conta? <a href="#" onclick="baixar('{$id}')"><span class="text-verde">Sim</span></a></p>
		...
		<big><a href="http://api.whatsapp.com/send?1=pt_BR&phone=$whats&text=Ola, {$nome_pessoa} Lembrete de vencimento de pagamento no dia: {$data_vencF} no valor de: R$ {$valorF}." target="_blank" title="Abrir Whatsapp" class="{$classe_whats}"><i class="fab fa-whatsapp {$classe_cor_whats}"></i></a></big>
